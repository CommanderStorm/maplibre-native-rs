...
//!
//! On Linux the Ninja generator is used when `ninja` is on `PATH` (unless `CMAKE_GENERATOR` is set), else Make.
//!
//! Required libraries:
//! Fedora:
...
fn build_local(
    ...
    // Apple platforms require Ninja (darwin.cmake calls enable_language(Swift),
    // unsupported by Make). On Linux, use Ninja when available, else fall back to Make.
